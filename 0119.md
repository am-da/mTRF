...
32の異なる脳波（EEG）チャネルにわたるTime-Resolved Field（TRF）の分散を計算。これにより、グローバル フィールド パワー（GFP）の測定値が推定される。

GFPは、頭皮全体での反応強度の尺度であり、各時間差において頭皮上の異なる位置でのEEG反応の強度の変動を捉えます。

文章は、GFPの測定に基づく時間プロファイルに焦点を当てており、特に~50、~80、~140 msで3つの明確なTRFコンポーネントが観察されたことを示しています。
これらのコンポーネントは、異なる時間ラグで音声刺激に対する脳波の反応のピークを示しています。
このような時間ラグごとの反応の変動を把握することで、音声刺激が脳波にどの程度の時間的な遅れで影響を与えているかを理解するのに役立ちます。

mTRF は -150 ミリ秒と 450 ミリ秒のラグの間で計算されていた。
